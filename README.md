...
Build an express app that let's you create, read, update, and delete
...
- Practice adding delete routes 
- Practice adding an edit show route 
- Practice adding a destroy route

#### Prerequisites

- Express Basics (Create/Read/Update/Delete & Static Assets)
...
- delete an income/expenditure item
- update an income/expenditure item
...
- As with non-deliverables homework will be reviewed upon student request 
...
#### Delete
- `show.ejs` - add a button / link that will 'delete' an entry in your budget
- use 'method-override' syntax in your form/link to hit your delete route
- in `server.js` A new route that is a `delete` route
- Remove the selected indexed item from the `/models/budget.js` data
- There are many ways to do this (filter, splice, for loop)
- then redirect to the index (`res.redirect('/)`)
- when you redirect to your index page, your previous item should not appear

#### Update
- A new route that is a `put` route
- A new view that displays a form for the selected entry, with pre-populated input fields
- verify the form action links to the put route
- first just console.log the `req.body`
- once the `req.body` is what you'd an expect (an object with keys that match our data in our `models/budget.js` and values that were entered in your form) `.splice()` the `req.body` to your `Budget` at the correct index position, or reassign that positions value to the `req.body`
- then redirect to the show page for the selected entry
- when you redirect to your show page, your updated item should appear
